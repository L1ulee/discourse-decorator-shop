...
module GamifiedShop
  class ShopItem < ActiveRecord::Base
    ...
    belongs_to :decoration_asset, class_name: "GamifiedShop::DecorationAsset", optional: true
    ...
    def self.claim_stock!(item_id)
      where(id: item_id).where("stock IS NULL OR stock > 0").update_all(
    ...
    def self.release_stock!(item_id)
      where(id: item_id).where.not(stock: nil).update_all("stock = stock + 1, updated_at = NOW()")
    end
